...
export function About() {
  return <>
    ...
    <section className="tight"><h2>Clients, by sector</h2><div className="client-groups">{sectors.map(([sector,clients]) => <div className="grp" key={sector}><div className="h">{sector}</div><div className="clients-grid">{clients.map(client => <span key={client}>{client}</span>)}</div></div>)}</div></section>
  </>;
}
